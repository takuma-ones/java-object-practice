...
public class Monster3 {
	...
	private String wazaNm      ;               // わざ名
	private String wazaDmgRate ;               //ダメージ倍率
	...
	protected Monster3(String tr, String nm){
		this();     
	...
	protected Monster3(String tr, String nm, int lev){
		this(tr, nm); 
		if(lev > 1){
			levelUp(lev - 1);
		}
	...
	public String toString() {
		return """
			...
			技名：%s
			技倍率：%s
			""".formatted(name, lv, hp, hpMax, character, trainer, atk, def, spd, wazaNm, wazaDmgRate);
	}
	...
	public int useWaza(){
		BigDecimal bgatk = new BigDecimal(atk);
		BigDecimal bgdmg = new BigDecimal(wazaDmgRate);
		int        dmg   = (bgatk.multiply(bgdmg)).intValue(); // 攻撃力×技倍率
	...
	public int damaged(int orgDm){
		...
		BigDecimal dmRate = bdDmCr1.divide(
			bdDmCr1.add(
				bdDef.divide(bdDmCr120, 2, RoundingMode.DOWN)
			),
			2,
			RoundingMode.DOWN
		);
